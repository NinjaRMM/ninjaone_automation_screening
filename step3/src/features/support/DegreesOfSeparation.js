...
class DegreesOfSeparation {
  ...
  bfs() { // Breadth-First Search
    ...
    if (!start || (typeof start === 'string' || typeof end === 'string')) {
      !start && (message = `${this.startActor} did not star in a movie in the data provided.`)
    } else {
      const queue = []
      start.searched = true
      queue.push(start)
  ...
  setText({ actorsPath, moviesPath, message }) {
    ...
    const mainText = message
      || `There ${actorsPath.length < 3 ? 'is' : 'are'} ${actorsPath.length - 1} degree${actorsPath.length < 3 ? '' : 's'} of separation between ${actorsPath[0]} and ${actorsPath[actorsPath.length - 1]}`
    this.setNumOfDegreesText(mainText)
    for (let i = 0; i < moviesPath.length; i++) {
      textsList += `${actorsPath[i]} starred with ${actorsPath[i + 1]} in ${moviesPath[i]}\n`
    }
