...
Then(
  "I should see the {string} of separation from {string}",
  function (numberOfDegrees, actorName) {
...
Then(
  "I see a list of movies describing the degree steps",
  function (docString) {
...
When(
  "I provide two actors' names as a parameters {string}, {string}",
  function (startActor, endActor) {
...
Then(
  "I should see the {string} of separation between the two actors",
  function (numberOfDegrees) {
    this.bfs();
    const numberOfDegreesText = this.getNumOfDegreesText();
    expect(numberOfDegreesText).eq(numberOfDegrees);
  }
...
When(
  "I provide a name not in the data as a parameter {string}",
  function (actorName) {
    this.setStartNode(actorName);
  }
);
...
Then(
  "I should see a {string} stating that name did not star in a movie",
  function (message) {
    this.bfs();
    const numberOfDegreesText = this.getNumOfDegreesText();
...
When(
  "I provide two actors' names one known, and one not in the data as a parameters {string}, {string}",
  function (startActor, endActor) {
    this.setStartNode(startActor);
    this.setEndNode(endActor);
  }
